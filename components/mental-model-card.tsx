...
interface MentalModelCardProps {
  model: MentalModel
  isActive?: boolean
  onToggle?: (modelId: string) => void // Make onToggle optional
...
export function MentalModelCard({ model, isActive = false, onToggle, showRelevance = false }: MentalModelCardProps) {
  return (
    <Card
      className={`p-5 transition-all duration-200 w-full ${isActive ? "ring-2 ring-blue-500 bg-blue-50" : "hover:shadow-md"}`}
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {onToggle && ( // Conditionally render the button
            <Button variant={isActive ? "default" : "outline"} size="sm" onClick={() => onToggle(model.id)}>
      ...
      <div className="space-y-2">
        ...
        <div className="text-xs text-blue-600 font-medium">Case study</div>
      </div>
    </Card>
  )
}